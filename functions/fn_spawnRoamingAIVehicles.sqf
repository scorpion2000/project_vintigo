while {roamingAIVehicleCount <= 4} do {
	...
	_vehicle = "I_G_Offroad_01_armed_F" createVehicle getPos ([getPos _randomSpawnLoc vectorAdd [random [-500, 0, 500], random [-500, 0, 500], 0], 200] call BIS_fnc_nearestRoad);
	...
	while {_rnd > 0} do {
		_wp = _group addWaypoint [getPos [getPos (selectRandom mapLocations) vectorAdd [random [-500, 0, 500], random [-500, 0, 500], 0], 200] call BIS_fnc_nearestRoad, 0];
